...
public class BindCommandButton {
    public int x,y,width,height;

    private MinecraftClient mc = MinecraftClient.getInstance();

    private BindCommandWindow parent;
    public BindCommand mod;
    ...
    public BindCommandButton(BindCommand mod, int x, int y, int width, int height, BindCommandWindow parent) {
        this.x = x;
        this.y = y;
        this.width = width;
        this.height = height;
    ...
    public void render(MatrixStack matrixStack, int mouseX, int mouseY) {
        ...
        mc.textRenderer.draw(matrixStack, mod.getCommandName(), x + width/2 - (mc.textRenderer.getWidth(mod.getCommandName())/2) , y + 3, -1);
        ...
        if (listening) {
            ...
            mc.textRenderer.draw(matrixStack, "Press 'DEL' to unbind " + mod.getCommandName() + (mod.getKeyCode() > -1 ? " (" + GLFW.glfwGetKeyName(mod.getKeyCode(),
                    GLFW.glfwGetKeyScancode(mod.getKeyCode())) + ")" : ""),
                    0 - mc.textRenderer.getWidth("Press 'DEL' to unbind " + mod.getCommandName() + (mod.getKeyCode() > -1 ? " (" + GLFW.glfwGetKeyName(mod.getKeyCode(),
                    GLFW.glfwGetKeyScancode(mod.getKeyCode())) + ")" : "")) / 2, 7 - mc.textRenderer.fontHeight, 0xffffffff);

            if(keyDown != -1) {
                if (keyDown != GLFW.GLFW_KEY_DELETE) {
                    LemonLogger.infoMessage("Bound ","'" + mod.getCommandName() + "'" + " to '" + GLFW.glfwGetKeyName(keyDown, GLFW.glfwGetKeyScancode(keyDown)) + "'");
                    mod.setKeyCode(keyDown);
                    FileManager.saveModules();
                } else {
                    LemonLogger.infoMessage("Unbound ","'" + mod.getCommandName() + "'");
