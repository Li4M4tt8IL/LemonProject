...
public class PlayerListHudMixin extends DrawableHelper {
    ...
    @Shadow
    private final InGameHud inGameHud = client.inGameHud;
    @Shadow
    private Text footer;
    @Shadow
    private Text header;
    ...
    private static final Ordering<PlayerListEntry> ENTRY_ORDERING = ((IPlayerListHud) mc.inGameHud.getPlayerListHud()).getOrdering();
    ...
    public void render(MatrixStack matrices, int i, Scoreboard scoreboard, @Nullable ScoreboardObjective objective) {
        ...
        while(var9.hasNext()) {
            ...
            o = this.client.textRenderer.getWidth(this.getPlayerName(playerListEntry));
            j = Math.max(j, o);
        ...
        if (list2 != null) {
            ...
            fill(matrices, var10001, var10002, var10003, u + var10005 * 9, -2147483648);

            for(var37 = list2.iterator(); var37.hasNext(); u += 9) {
                orderedText3 = (OrderedText)var37.next();
                z = this.client.textRenderer.getWidth(orderedText3);
                this.client.textRenderer.drawWithShadow(matrices, orderedText3, (float)(i / 2 - z / 2), (float)u, -1);
                this.client.textRenderer.getClass();
            }
        ...
        fill(matrices, i / 2 - v / 2 - 1, u - 1, i / 2 + v / 2 + 1, u + n * 9, -2147483648);
        ...
        for(int y = 0; y < m; ++y) {
            ...
            if (y < list.size()) {
                ...
                if (bl) {
                    ...
                    if(Main.getNames().contains(playerListEntry2.getProfile().getId().toString())) {
                        int af = 8 + (bl2 ? 8 : 0);
                        int ag = 8 * (bl2 ? -1 : 1);
                        client.getTextureManager().bindTexture(new Identifier(Main.MOD_ID, "16x16.png"));
                        DrawableHelper.drawTexture(matrices, ab + 9, ac, 8, 8, 40.0F, (float)af, 8, ag, 64, 64);
                        ab += 9;
                    }
